...
public sealed class WcDataCatalog
{
    ...
    public WcDataCatalog(IWebHostEnvironment environment, ILogger<WcDataCatalog> logger)
    {
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken = default)
    {
        ...
        var path = Path.Combine(_environment.ContentRootPath, "Data", "wc_data.json");
        await using var stream = File.OpenRead(path);
        _document = await JsonDocument.ParseAsync(stream, cancellationToken: cancellationToken);
        _logger.LogInformation("Loaded War Chronicle data from {Path}", path);
    }
